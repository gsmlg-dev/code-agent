...
Extension tools (`install_extension`, `list_extensions`, etc.) are available
only when the MCP server is started with the `--categoryExtensions` flag. If
these tools are unavailable, stop and ask the user to update their MCP server
configuration and restart the MCP server (or AI client).
