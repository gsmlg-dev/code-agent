---
name: cmd-update-speckit-plugin
...
LOG_FILE="$(git rev-parse --show-toplevel)/.agents/skills/cmd-update-speckit-plugin/UPDATES.md"
```
...
git diff --name-status "$BASE_COMMIT" -- plugins/speckit/
```

Append a dated entry to `"$LOG_FILE"` with the source URL/ref, `BASE_COMMIT`, target, Added/Modified/Deleted counts, and each changed path. Include this restore command for deleted paths: `git restore --source <base-commit> -- plugins/speckit/<path>`. Stage the log together with plugin changes and commit them in the same commit.
